...
# Saving EEPROM data when bootloading on STM32

![](https://substackcdn.com/image/fetch/$s_!ZALq!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F12cbcac3-de9e-4981-a5f2-c6f176d3d19a_400x402.jpeg)
...
**Concepts:** [[concepts/location|Location]]
...
> _The read or written values are 16-bit, not 8-bit. This is because the STM32 FLASH library only allows operations on word boundaries (as far as I understand)._
...
[![](https://substackcdn.com/image/fetch/$s_!ZALq!,w_1456,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F12cbcac3-de9e-4981-a5f2-c6f176d3d19a_400x402.jpeg)](<https://substackcdn.com/image/fetch/$s_!ZALq!,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fsubstack-post-media.s3.amazonaws.com%2Fpublic%2Fimages%2F12cbcac3-de9e-4981-a5f2-c6f176d3d19a_400x402.jpeg>)
...
So, what does this mean? 123 [is the ASCII character ](<http://www.asciitable.com/>)`{`, and 456 in hex is `0x1C8`, so it looks like _we found the data!_ I’m not totally sure why it is not in the first 4 bytes, but let’s ignore that for now. The python code above saved the EEPROM data to the file `eeprom.bin`.
...
This modified version of [stm32loader.py](<https://github.com/avikde/koduino/blob/master/stm32/system/stm32loader.py#L526>) included with [Koduino](<https://github.com/avikde/koduino>), contains the conceptually simple operation:
  ...
  1. Read the EEPROM data _before_ extended erase
